...
public final class TrollheimPlugin extends OptionHandler {
	...
	public Plugin<Object> newInstance(Object arg) throws Throwable {
		// Keldgraim getting inside the cave
		ObjectDefinition.forId(5008).getConfigurations().put("option:enter", this);
		// Keldgraim getting outside the cave
		ObjectDefinition.forId(5014).getConfigurations().put("option:enter", this);
		ObjectDefinition.forId(5973).getConfigurations().put("option:go-through", this);
		ObjectDefinition.forId(5998).getConfigurations().put("option:go-through", this);
	...
	public boolean handle(final Player player, Node node, String option) {
		switch (node.getId()) {
		case 5014:
		    player.teleport(Location.create(2730, 3712, 0));		    
			flagDoor(player, false);
			ClimbActionHandler.climb(player, new Animation(748), Location.create(2730, 3713, 0));
			player.getDialogueInterpreter().open(70099, "You squeeze through the hole leading outside Keldagrim caves");
			return true;
		case 5973:
			player.teleport(Location.create(2892, 10225, 0));
		}
		...
		case "enter":
			...
			case 4500:
				...
				break;
			case 5008:
				player.teleport(Location.create(2773, 10162, 0));
				break;
			case 3723:
